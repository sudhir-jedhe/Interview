...
const Child = ({ inputRef }) => <input ref={inputRef} disabled />;
```
...
const MyComponent = () => {
  ...
  return (
    <>
      ...
      />
      <div>
        {maxLength - textareaRef.current?.value?.length} characters remaining
      </div>
    </>
  );
};
...
This is an excellent, comprehensive list of fundamental React interview questions and code snippets! It covers almost every core concept needed for React front-end interviews—from basic JSX and props to advanced hooks, portals, performance optimization, and error handling.

To make this collection even more robust, here are a few **important corrections, optimizations, and best practices** for some of the snippets above that interviewers often look out for:

---

### **Key Improvements & Best Practices**

#### **1. Question 1 (Display Dynamic HTML / DangerouslySetInnerHTML)**

- **Note:** Standard JSX `{name}` renders plain text/strings. If an interviewer asks how to render raw, dynamic **HTML strings** (e.g., HTML from a CMS or API like `"<h1>Title</h1>"`), you must use `dangerouslySetInnerHTML`:

```jsx
const RawHtmlComponent = ({ htmlString }) => {
  return <div dangerouslySetInnerHTML={{ __html: htmlString }} />;
};

```

#### **2. Question 5, 8, & 12 (Using `key={index}` in Lists)**

- **Best Practice:** Using `index` as a key can cause rendering bugs and state issues if items are added, removed, or reordered. It’s always best to use a unique ID (e.g., `item.id`):
...
#### **3. Question 21 (`useReducer` Hook Syntax Correction)**

- In your example, the `useReducer` import was slightly misformatted inside the snippet. Here is the clean version:
...
#### **4. Question 22 (Ref Forwarding to Child Components)**

- Standard DOM `ref` passing across custom components often uses `React.forwardRef` (or direct prop passing in React 19+). Here is the standard `forwardRef` pattern:

```jsx
import React, { useRef, forwardRef } from "react";

const Child = forwardRef((props, ref) => (
  <input ref={ref} disabled placeholder="I am child input" />
));
...
const Parent = () => {
  ...
  return (
    <div>
      <Child ref={inputRef} />
      <button onClick={enableInput}>Enable & Focus Input</button>
    </div>
  );
};
...
#### **5. Question 27 (Character Count using Controlled State vs. Ref)**

- Reading `.current.value` during render inside JSX (`maxLength - textareaRef.current?.value?.length`) doesn't always trigger a re-render automatically unless state changes. The cleanest way is using state:
...
#### **6. Question 38 (React Portal Guard)**

- Always verify that `document.getElementById('popup-root')` exists before calling `createPortal`, or append it dynamically to avoid `null` target element errors:
...
### **Summary Table of React Lifecycle vs. `useEffect` Equivalent**

For **Question 39**, here is a quick cheat sheet for interviews:
...
| `shouldComponentUpdate` | Handled by `React.memo()` or `useMemo()`                  |
